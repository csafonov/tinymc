...
public class PIDData {
	...
	public double getKp(int indCrl)
	{
		return Crl[indCrl  + nCrl];
	}
	public double getKi(int indCrl)
	{
		return Crl[indCrl  + nCrl*2];
	}
	public double getKd(int indCrl)
	{
		return Crl[indCrl];
	}
	public double getW(int indCrl)
	{
		return Crl[indCrl  + nCrl*3];
	}
	...
	public void prepKarman(double[] drawbuffer, int buffer_len, int indCrl) {
		...
		double [] imPole = new double[numFreqs];
			double [] reProp = new double[numFreqs];
			double [] imProp = new double[numFreqs];
...
*/
		/*
			CtrlHelper.pole1(numFreqs, rePole, imPole, Fw, wPole);
			CtrlHelper.mult(numFreqs, re , im , rePole, imPole);
*/


			CtrlHelper.ones(numFreqs, reProp, imProp);//KP
			CtrlHelper.mult(numFreqs, reProp, imProp, Kp);

			CtrlHelper.ones(numFreqs, reInt, imInt);
			CtrlHelper.mult(numFreqs, reInt, imInt, Ki);
			CtrlHelper.div(numFreqs, reInt, imInt, reS, imS);

			CtrlHelper.add(numFreqs, reInt, imInt, reProp, imProp );
			CtrlHelper.pole1(numFreqs, rePole, imPole, Fw, wPole);

			CtrlHelper.mult(numFreqs, reInt, imInt ,rePole, imPole);
			CtrlHelper.mult(numFreqs, re , im  ,reInt, imInt);// P*(KP+KI/s)*w/(s+w)

		 offs = PlotNic.AddNicholsPlot(drawbuffer, offs, color,numFreqs,re,im);
		//	offs = PlotNic.AddNicholsPlot(drawbuffer, offs, color,numFreqs,reInt, imInt);
		}
