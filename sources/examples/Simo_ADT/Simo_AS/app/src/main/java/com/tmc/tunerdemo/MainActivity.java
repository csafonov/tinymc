...
public class MainActivity extends Activity implements Runnable
{
	...
	void getPIDresult()
	{
		...
		for (int k=0;k< (1+2*NumPlants)*NumFreqs;k++  )
		{
			plants[k] = pidgetGetPlantVal(k);
		}
	    int NumCrl = pidgetNumCrl();
	    double [] crl = new double[ 4 *  NumCrl ];
		for (int m=0;m< 4 *  NumCrl;m++  )
		{
			crl[m] = pidgetGetCrlVal(m);
		}
	    //pidgetGetCrl(crl);
...
//    		double y[];
 ...
 ///   	
	...
	public native double pidgetGetPlantVal(int ind);
