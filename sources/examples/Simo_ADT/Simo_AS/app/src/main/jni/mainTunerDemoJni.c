...
jdouble Java_com_tmc_tunerdemo_MainActivity_pidgetGetPlantVal(
                JNIEnv *env,
                jobject callingObject,
                jint ind)
{
	int kk=ind;
	return g_PIDData.plants[kk];
}
...
/**
    \brief Implements the native MainActivity method
        public native double pidgetGetCrlVal(int ind);
*/
jdouble Java_com_tmc_tunerdemo_MainActivity_pidgetGetCrlVal(
                JNIEnv *env,
                jobject callingObject,
               jint ind)
{
	int kk=ind;
	return g_PIDData.crl[kk];
}
