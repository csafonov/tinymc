...
/*  -- LAPACK auxiliary routine (version 3.1) --   
    ...
    static integer inf, kdu, nho, nve, kwh, nsr, nwr, kwv, ndfl, kbot, nmin;
...
/*        ==== ZLAHQR/ZLAQR0 crossover point ==== */
	...
	nmin = ilaenv_(&c__12, "ZLAQR0", jbcmpz, n, ilo, ihi, lwork, (ftnlen)6,
                 (ftnlen)2);
	nmin = max(11,nmin);

/*        ==== Nibble crossover point ==== */

	nibble = ilaenv_(&c__14, "ZLAQR0", jbcmpz, n, ilo, ihi, lwork, (ftnlen)6,
                 (ftnlen)2);
	nibble = max(0,nibble);
...
/*        ==== Accumulate reflections during ttswp?  Use block   
	...
	kacc22 = ilaenv_(&c__16, "ZLAQR0", jbcmpz, n, ilo, ihi, lwork, (ftnlen)6,
                 (ftnlen)2);
	kacc22 = max(0,kacc22);
	kacc22 = min(2,kacc22);
...
/* Computing MIN */
			...
			if (nw >= nh - 1) {
			    ...
			    if ((d__1 = h__[i__2].r, abs(d__1)) + (d__2 = 
				    d_imag(&h__[kwtop + (kwtop - 1) * h_dim1])
				    , abs(d__2)) > (d__3 = h__[i__3].r, abs(
				    d__3)) + (d__4 = d_imag(&h__[kwtop - 1 + (
				    kwtop - 2) * h_dim1]), abs(d__4))) {
				++nw;
			    }
			}
		    }
