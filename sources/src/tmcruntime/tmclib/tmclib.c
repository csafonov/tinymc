			...
			for(n=0;n<N;n++)
			{
				...
				else
				{
					...
					// HAZARD: protect against unsupported boolean addressing. Should be solved at compiler level.
						if (ind2 < 0)
						{
							_tmcRaiseException(err_bad_index, s_module, "_tmcGetByIndex", "unsupported boolean access detected like x( condition result )", 2, I1, src);
						}
					}
					else
					{
...
{	/*
	...
	long M,N,MN,k;
		...
		_tmcRaiseException(err_must_be_real,s_module,"double()","Operand to double should be matrix or string.",1,src);
...
/// Additional exports
long tmcGetM(tmsMatrix *X)
{
	return _tmcGetM(X);
}
long tmcGetN(tmsMatrix *X)
{
	return _tmcGetN(X);
}
long tmcGetType(tmsMatrix *X)
{
	return _tmcGetType(X);
}
short tmcHasIm(tmsMatrix *X)
{
		return _tmcHasIm(X);
}
double tmcScalarVal(tmsMatrix *X)
{
		return _tmcScalarVal(X);
}
double tmcMatrixValRe(tmsMatrix *X,long m,long n)
{
		return X->value.complx.rData[m+n*_tmcGetM(X)];
}
double tmcMatrixValIm(tmsMatrix *X,long m,long n)
{
		return X->value.complx.iData[m+n*_tmcGetM(X)];
}
double* tmcMatrixPRe(tmsMatrix *X,long m,long n)
{
		return X->value.complx.rData;
}
